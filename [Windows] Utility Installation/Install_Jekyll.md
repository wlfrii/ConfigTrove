...
如果发现网页上的效果与实际的本地编译的效果不同，则确认下
**1. 确认 _config.yml 设置**

检查 `_config.yml` 文件，确保以下设置正确：
+ __Base URL__：如果你的 GitHub Pages 不是在根目录下，确保配置 baseurl 属性。
```yaml
baseurl: "/repository-name"  # 替换为你的仓库名
```
+ __URL__：确保 url 设置正确。
```yaml
url: "https://username.github.io"  # 替换为你的 GitHub 用户名
```

**2. 静态资源路径**

在 Jekyll 中，如果使用了相对路径，确保在引用静态资源（如 CSS、JS 文件和图片）时考虑 baseurl。例如：
